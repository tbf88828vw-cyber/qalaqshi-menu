...
export const DEFAULT_SETTINGS = {
  ...
  heroVideo: { src: '/media/hero-v3.mp4', poster: '/media/hero-v3-poster.webp', blur: 'data:image/webp;base64,UklGRnAAAABXRUJQVlA4IGQAAAAQAgCdASoQABAAA4BaJagCdAEbu+nXnZ2gAP7ys8criI8HNLE2nc7ForTOTt+GezZifBE0q4gLkowAftAuiKR7zAC4YnedMH9N7yaqZQ3n7XL65ZQL6COvsnMm/eia9xIwAAAA', soft: true },
};
...
function cleanSettings(raw, prev) {
  ...
  if ('heroVideo' in raw) {
    ...
    if (s.heroVideo && ['/media/hero-bg.mp4', '/media/hero-v2.mp4'].includes(s.heroVideo.src)) s.heroVideo = { ...DEFAULT_SETTINGS.heroVideo };
  }
  return s;
}
...
export function applyOp(doc, op, now = Date.now()) {
  ...
  switch (op.type) {
    ...
    case 'items.media': {
      ...
      note = n === 1 && Object.keys(op.map || {}).length === 1 ? 'Медиа сохранено' : 'Фото перенесены: ' + n;
      break;
    }
    case 'items.place': {
