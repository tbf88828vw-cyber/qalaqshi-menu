...
const SEED_MEDIA = {
  19: {
    type: 'video', src: '/media/pkhali-720.mp4', srcLow: '/media/pkhali-480.mp4', poster: '/media/pkhali-poster.webp',
    blur: 'data:image/webp;base64,UklGRoAAAABXRUJQVlA4IHQAAADQAwCdASoMABUAPu1iqk2ppaQiMAgBMB2JQBOmUABTh1TXTuCdEKgA/oNZS2Zir9AsjZi1JvrqdXXbQBCOL+f/NAY5nc9FnWfoRiSVCSgcAKT4XatcijE0y5oCMtdQnTUFyqp79b+WUNs2jD1dYfJrjMZAAA==',
    w: 720, h: 1280, dur: 6.2,
  },
  85: {
    type: 'video', src: '/media/mtsvadi-720.mp4', srcLow: '/media/mtsvadi-480.mp4', poster: '/media/mtsvadi-poster.webp',
    blur: 'data:image/webp;base64,UklGRoAAAABXRUJQVlA4IHQAAACwAwCdASoMABUAPu1iqU2ppaOiMAgBMB2JQBdgA+9PXpBGd2DegAD9N6HAExiZt0sEIoYP6lTBMFgTeZKmz1bLYaoLU6/Hwy2gSdiuPd8ckBbn905ltJ9sxUdzBbM4LD+gSI9sCpoaJbO+GYGGVnTs/a8AAA==',
    w: 720, h: 1280, dur: 6.8,
  },
};
...
const SEED_TEXT = {
  ...
  85: {
    desc: {
      ...
      ka: 'წვნიანი ღორის ხორცი ნახშირზე, მარინადში ჩალბილი ხახვით.',
    },
    ingr: {
...
export const DEFAULT_SETTINGS = {
  ...
  heroVideo: { src: '/media/hero-bg.mp4', poster: '/media/hero-poster.webp', blur: 'data:image/webp;base64,UklGRn4AAABXRUJQVlA4IHIAAACQAwCdASoMABUAPu1iqU2ppaOiMAgBMB2JQBdgAdDpUDV9ZSagAP03ocATGJm5qQLeKJjL9i9B68f1Z4bmjGWOwzZEKCdb7f4QXvC2/JhyLP7hd9p0Di8e7pDoWzOCw/oEiPbAqaGif27gLg7EjCSUAAA=' },
};
...
function cleanSettings(raw, prev) {
  ...
  for (const k of ['address', 'eyebrow', 'tagline', 'specialsTitle']) if (k in raw) s[k] = i18n(raw[k], 160);
  if ('heroVideo' in raw) {
    const m = cleanMedia(raw.heroVideo ? { type: 'video', ...raw.heroVideo } : null);
    s.heroVideo = m ? { src: m.src, poster: m.poster || '', blur: m.blur || '' } : null;
  }
  return s;
}
...
export function publicView(doc) {
  ...
  return {
    ...
    categories: cats.map((c) => ({ id: c.id, section: c.section, group: c.group, display: c.display, name: c.name })),
    items,
    specials,
    badges: BADGES,
  };
...
export function collectMediaUrls(doc, set = new Set()) {
  const add = (m) => { if (m) for (const k of ['src', 'srcLow', 'poster']) if (m[k]) set.add(m[k]); };
